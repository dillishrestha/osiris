// created on 13/11/2008 at 11:10 a
// Hospital Santa Cecilia
// Monterrey - Mexico
//
// Autor    	: Ing. Jesus Buentello Garza
// 				  
// Licencia		: GLP
//////////////////////////////////////////////////////////
//
...
namespace osiris
{
	public class cargos_hemodialisis
	{
		...
		public int idtipointernamiento = 840;				// Toma el valor del tipo de internamiento
		public string descripinternamiento = "HEMODIALISIS";  	// Toma la descripcion del internamiento
		...
		public int idsubalmacen = 14;   // Sub-Almacen de Hemodialisis
		...
		public cargos_hemodialisis(string LoginEmp, string NomEmpleado_, string AppEmpleado_, string ApmEmpleado_, string _nombrebd_ ) 
		{
			...
			cargos_hospital.Title = "HEMODIALISIS";
							...
							if (treeViewEngineServicio.GetIterFirst (out iter)){
								if ((bool)lista_de_servicios.Model.GetValue (iter,14)==false){
									...
									try{
										...
										comando2.CommandText = "SELECT to_char(hscmty_catalogo_almacenes.id_producto,'999999999999') AS codProducto,"+						
													"to_char(stock,'999999999.99') AS stock_subalmacen "+				
													"FROM hscmty_catalogo_almacenes "+
													"WHERE hscmty_catalogo_almacenes.id_almacen = '"+this.idsubalmacen.ToString().Trim()+"' "+
													"AND hscmty_catalogo_almacenes.id_producto = '"+double.Parse((string) lista_de_servicios.Model.GetValue(iter,2))+"'; ";
										...
										if (verifica_stock == true){
											...
											conexion1 = new NpgsqlConnection (connectionString+nombrebd);
					 						try{
												...
												comando1.CommandText = "UPDATE hscmty_catalogo_almacenes SET stock  = stock - '"+(float) lista_de_servicios.Model.GetValue(iter,1)+"' "+
			...
			if (miResultado == ResponseType.Yes){
				...
				try{
					...
					comando.CommandText = "UPDATE hscmty_erp_cobros_enca "+
									...
									"motivo_alta_paciente = '"+causa_de_alta_paciente+"', "+
									"observaciones_de_alta = ' "+observacionesalta+" ' "+
				...
				if (!(bool) lista_de_servicios.Model.GetValue (iter,14)){
					...
					//toma_a_pagar = sub_total - totaldescuento;
 					...
 					if (LoginEmpleado =="DOLIVARES" || LoginEmpleado =="HVARGAS" || LoginEmpleado =="JBUENTELLO" || LoginEmpleado == "N000426" ){
 						...
 						// Validar que el producto sea del mismo almacen cargado 
 						if ((int) lista_de_servicios.Model.GetValue (iter,13) == idtipointernamiento){
							...
							msgBox.Destroy();
		 					...
		 					if (miResultado == ResponseType.Yes){
								...
								try{
						        	...
						        	comando.CommandText = "UPDATE hscmty_catalogo_almacenes "+
	 						...
	 						MessageDialog msgBox = new MessageDialog (MyWin,DialogFlags.Modal,
										MessageType.Error,ButtonsType.Ok,"El producto a devolver, NO Es de este CENTRO DE COSTO");
							msgBox.Run();		msgBox.Destroy();
	 					}
 					}else{
 						MessageDialog msgBox = new MessageDialog (MyWin,DialogFlags.Modal,
		...
		{
			Console.WriteLine("almacen  "+this.idsubalmacen.ToString());
			subtotal_al_15 = 0;
			subtotal_al_0 = 0;
			total_iva = 0;
			totaldescuento = 0;
		...
		{	
			...
			try{
				...
				comando.CommandText = "SELECT descripcion_producto,to_char(hscmty_erp_cobros_deta.id_producto,'999999999999') AS idproducto, "+
							...
							"eliminado,"+
							"to_char(hscmty_erp_cobros_deta.id_almacen,'9999999999') AS idalmacen_detacobros,"+
							...
							"ORDER BY to_char(hscmty_erp_cobros_deta.fechahora_creacion,'yyyy-MM-dd HH24:mi:ss');";
 		...
 		void llenando_lista_de_productos()
 		{
			...
			try{
				...
				comando.CommandText = "SELECT to_char(hscmty_productos.id_producto,'999999999999') AS codProducto,"+
						...
						"AND hscmty_catalogo_almacenes.id_almacen = '"+this.idsubalmacen.ToString().Trim()+"' "+
