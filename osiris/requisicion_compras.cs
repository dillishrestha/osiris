...
namespace osiris
{
	public class requisicion_materiales_compras
	{
			...
			if (checkbutton_nueva_requisicion.Active == true){
				...
				msgBox.Destroy();
		 		if (miResultado == ResponseType.Yes){
					...
					editar = false;										
					//this.entry_requisicion.Text = ultimarequisicion().ToString().Trim();					
		...
		void creacion_del_combobox(int tipocombobox,string solicitadopor,string concargoa,bool ordinaria_,bool urgente_,string tiporequisicion_)
		{
 		...
 		// Buscando el ultimo numero de requisicion
 		int ultimarequisicion()
 		{
 			idrequisicion = 0;
 			NpgsqlConnection conexion;
			conexion = new NpgsqlConnection (connectionString+nombrebd );
			...
			try{
				...
				if ((bool) lector.Read()){
					idrequisicion = (int) lector["id_requisicion"] + 1;
				}else{
					idrequisicion = 1;
				}
				lector.Close ();
