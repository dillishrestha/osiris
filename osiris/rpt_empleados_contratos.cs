...
namespace osiris
{
	...
	public string connectionString = "Server=localhost;" +
		...
		public reportes_empleados(string LoginEmp_, string NomEmpleado_, string AppEmpleado_, string ApmEmpleado_, string _nombrebd_,string tipo_reporte_)
		{
			...
			button_buscar_empleado.Clicked += new EventHandler(on_buscar_empleado_clicked);
			...
			if (tipo_reporte_ == "Contrato"){
				...
				this.button_contrato.Visible = true;
				this.button_buscar_empleado.Visible=false;
				this.check_todos.Active = true;
			}
		    ...
		    if (tipo_reporte_ == "Bajas"){
                ...
                this.button_buscar_empleado.Visible=true;			
			}
		...
		void on_buscar_empleado_clicked (object sender, EventArgs args)
		{
			...
			button_selecciona.Clicked += new EventHandler(on_selecciona_empleado_busqueda_clicked);
        ...
        void llena_lista_empleados()
		{
			...
			try{
				...
				//string toma_edad;
				while (lector.Read()){
					treeViewEngineBuscabaja.AppendValues ((string) lector["id_empleado"], 
        ...
        void crea_treeview_busqueda()
		  {
			treeViewEngineBuscabaja = new TreeStore(typeof(string),typeof(string),typeof(string),typeof(string),typeof(string));
			lista_de_empleados.Model = treeViewEngineBuscabaja;
			
			lista_de_empleados.RulesHint = true;
			
			lista_de_empleados.RowActivated += on_selecciona_empleado_busqueda_clicked;  // Doble click selecciono paciente*/
			...
			col_idEmpleado.SortColumnId = (int) Column3.col_idEmpleado;
			...
			col_Nombre1_Empleado.SortColumnId = (int) Column3.col_Nombre1_Empleado;
			...
			col_Nombre2_Empleado.SortColumnId = (int) Column3.col_Nombre2_Empleado;
			...
			col_app_Empleado.SortColumnId = (int) Column3.col_app_Empleado;
			...
			col_apm_Empleado.SortColumnId = (int) Column3.col_apm_Empleado;
		...
		void llena_empleado_debaja()
		{
			...
			try{
				...
				//hscmty_empleado.id_empleado
             	comando.CommandText = "SELECT hscmty_empleado.id_empleado,baja_empleado,nombre1_empleado,nombre2_empleado,apellido_paterno_empleado,apellido_materno_empleado,"+
					                 "causa_baja,notas_baja,to_char(fecha_de_baja,'YYYY/MM/dd') as fechadebaja,departamento,puesto "+
									"FROM hscmty_empleado_detalle,hscmty_empleado  " +
									"WHERE (hscmty_empleado.id_empleado = hscmty_empleado_detalle.id_empleado) "+
									"AND hscmty_empleado.id_empleado = '"+idempleadoseleccionado+"'; ";
				...
				while (lector.Read()){
					...
					treeViewEngineBusca.AppendValues (nombre_completo,(string) lector["id_empleado"],
										Convert.ToDateTime( fechadebaja).ToLongDateString() , (string) lector["causa_baja"],(string) lector["departamento"],(string) lector["puesto"]);
				}
			       ...
			       // conteo_lineas = 0;
			        ...
			        conteo_lineas = conteo_lineas + 1; //suma los renglones en cada linea
			              ...
			              switch (conteo_lineas)
                              {
                               case 1: 
                                  //registro,es el 1er punto y coma en el historial
                                 //Console.WriteLine(s);
                               break;
                               case 2:
                                 //fecha de registro, es el 2do punto y coma en el historial
                                 //Console.WriteLine(s);
                               break;
                               case 3:
                                     var_fecha = s;
								
			                         // valida la fecha, es el 3er punto y coma en el historial
                                     //    {fecha_valida = true;}  
