...
namespace osiris
{
	public class cargos_terapia_neonatal
	{
		...
		public int idtipointernamiento = 830;				// Toma el valor del tipo de internamiento
		public string descripinternamiento = "UNIDAD CUIDADOS INTENSIVOS NEONATOS";  	// Toma la descripcion del internamiento
		...
		public int idsubalmacen = 7;   // Sub-Almacen de Neonatos
		...
		public cargos_terapia_neonatal(string LoginEmp, string NomEmpleado_, string AppEmpleado_, string ApmEmpleado_, string _nombrebd_ ) 
		{
		...
		void llenado_de_productos_aplicados(string foliodeserv)
		{
			...
			try{
				...
				comando.CommandText = "SELECT DISTINCT (hscmty_erp_movcargos.folio_de_servicio),hscmty_erp_cobros_enca.folio_de_servicio AS foliodeatencion, "+
				            	...
				            	"to_char(to_number(to_char(age('"+DateTime.Now.ToString("yyyy-MM-dd hh:mm:ss")+"',hscmty_his_paciente.fecha_nacimiento_paciente),'MM') ,'99'),'99') AS meses, "+				            	
				...
				if(lector.Read()){
					...
					string mesespac = (string) lector["meses"];
					...
					// Validacion de Edad del paciente
					if( int.Parse(entry_edad.Text) == 0 && int.Parse(mesespac) <=1 ) {
						...
						button_busca_producto.Sensitive = false;
						MessageDialog msgBoxError = new MessageDialog (MyWinError,DialogFlags.DestroyWithParent,
						MessageType.Error,ButtonsType.Close,"El paciente NO cumple la edad necesaria para este modulo");
						msgBoxError.Run ();				msgBoxError.Destroy();
					}
				}
